...
export async function updateTemplate(id, { name, body, requireReply }) {
  // requireReply bersifat opsional di sini -- kalau tidak dikirim (undefined),
  // nilai lama di database TIDAK diubah (dipertahankan lewat COALESCE),
  // supaya endpoint ini tetap kompatibel dipanggil hanya dengan name+body.
  await pool.query(
