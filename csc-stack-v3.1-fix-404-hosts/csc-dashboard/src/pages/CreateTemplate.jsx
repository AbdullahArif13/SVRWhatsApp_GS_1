...
export default function CreateTemplate() {
  ...
  // v3.7: parameter True/False "Aktifkan Penggunaan Foto".
  //   true  -> template ini MEWAJIBKAN values.foto (URL gambar) diisi tiap
  //            kali dipakai lewat POST /api/send-message -- foto dikirim
  //            sebagai pesan WA terpisah setelah teks (lihat backend
  //            queueService.js).
  //   false -> (default) values.foto/keterangan diabaikan, tidak ada foto
  //            yang dikirim untuk template ini.
  const [usePhoto, setUsePhoto] = useState(false);
  // Contoh foto (opsional) SEKEDAR untuk pratinjau tampilan di panel
  // "Pratinjau" -- file ini TIDAK diupload/dikirim kemana-mana. Foto
  // sungguhan tiap pengiriman datang dari values.foto (URL) yang diisi
  // oleh sistem pemanggil (mis. CCTV), bukan dari sini.
  ...
  // Kalau toggle dimatikan lagi, buang juga contoh foto yang sempat dipilih
  // supaya panel Pratinjau tidak menampilkan foto dari state yang sudah
  // tidak relevan.
  useEffect(() => {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      await addTemplate({ name: templateName.trim(), body: bodyMessage, requireReply, usePhoto });
      navigate("/templates");
    } catch (err) {
    ...
    <Layout>
      ...
      <div className="flex gap-8 px-8 pb-8">
        ...
        <div className="flex flex-1 flex-col gap-4">
          ...
          {/* v3.7: toggle "Aktifkan Penggunaan Foto" -- kalau diaktifkan,
              template ini mewajibkan values.foto (URL gambar) diisi tiap
              kali dipakai lewat POST /api/send-message, dan foto tersebut
              dikirim sebagai pesan WA terpisah setelah teks. */}
          <div className="flex items-center justify-between rounded-lg border border-gray-200 px-4 py-3">
            <div>
              ...
              <p className="text-sm text-gray-400">
                {usePhoto ? (
                  <>
                    ...
                    foto akan dikirim sebagai pesan terpisah setelah teks ini.
                  </>
                ) : (
          ...
          {usePhoto && (
            <div className="-mt-2 flex flex-col gap-2 rounded-lg bg-amber-50 px-4 py-3 text-xs text-amber-700">
              <p>
                Sistem pemanggil (mis. CCTV AI) wajib mengisi <span className="font-semibold">values.foto</span>{" "}
                dengan URL gambar (JPG/PNG) saat memakai template ini, boleh ditambah{" "}
                <span className="font-semibold">values.keterangan</span> sebagai caption foto.
              </p>
              <label className="flex flex-col gap-1">
                ...
                <input
                  ...
                  onChange={(e) => setPhotoPreviewFile(e.target.files?.[0] ?? null)}
